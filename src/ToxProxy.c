...
// gcc parameter -DUNIQLOGFILE for logging to standardout = console
    ...
    snprintf(uniq_log_filename, sizeof(uniq_log_filename), "ToxProxy_%04d-%02d-%02d_%02d%02d-%02d,%06ld.log",
...
void toxProxyLog(int level, const char *msg, ...)
{
    ...
    sprintf(buffer, "%04d-%02d-%02d %02d:%02d:%02d.%06ld", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
            tm.tm_min, tm.tm_sec, tv.tv_usec);
    strcat(buffer, " [_] ");
...
void writeMessage(char *sender_key_hex, const uint8_t *message, size_t length)
{
    ...
    snprintf(timestamp, sizeof(timestamp), "%04d-%02d-%02d_%02d%02d-%02d,%06ld", tm.tm_year + 1900, tm.tm_mon + 1,
...
void friend_request_cb(Tox *tox, const uint8_t *public_key, const uint8_t *message, size_t length, void *user_data)
{
    ...
    toxProxyLog(2, "Got currently %zu friends. New friend request from %s with message: %s", friends, public_key_hex,
                message);
        ...
        if (is_master_friendnumber(tox, friend_number)) {
            if ((strlen((char *) message_text) == (strlen("fp:") + tox_public_key_hex_size))
                ...
                send_text_message_to_friend(tox, friend_number,
                                            "Sorry, but this command has not been understood, please check the implementation or contact the developer.");
            }
        } else {
